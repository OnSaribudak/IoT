...
void SendData(String DataPath, float value)
{
  Firebase.setFloat(fbdo, DataPath , value );
}

void GetData()
{
   int iVal = 0;
   String a = Firebase.getInt(fbdo, F("/test/int"), &iVal) ? String(iVal).c_str() : fbdo.errorReason().c_str();
}
